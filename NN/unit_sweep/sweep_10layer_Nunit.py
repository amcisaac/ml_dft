...
def train_model_10layer(X_train,y_train,X_test,y_test,N_units,acts,loss_fn,epochs,batch_size):
    model = Sequential()

    N_units1,N_units2,N_units3,N_units4,N_units5,N_units6,N_units7,N_units8,N_units9,N_units10 = N_units
    act1,act2,act3,act4,act5,act6,act7,act8,act9,act10 = acts
    ...
    layer6 = Dense(units=N_units6,activation=act6)
    model.add(layer6)

    layer7 = Dense(units=N_units7,activation=act7)
    model.add(layer7)

    layer8 = Dense(units=N_units8,activation=act8)
    model.add(layer8)

    layer9 = Dense(units=N_units9,activation=act9)
    model.add(layer9)

    layer10 = Dense(units=N_units10,activation=act10)
    model.add(layer10)
    ...
    # TRAIN THE MODEL
    #history = LossHistory()
    fit = model.fit(X_train, y_train, epochs=epochs, validation_data=(X_test,y_test), verbose=True,batch_size=batch_size),#callbacks=[history], verbose=True)
...
n_unit = 30
N_units = [n_unit,n_unit,n_unit,n_unit,n_unit,n_unit,n_unit,n_unit,n_unit,n_unit]
acts = ['relu','relu','relu','relu','relu','relu','relu','relu','relu','relu']
loss_fn = 'mean_squared_error'
epochs = 10
batch_size = 100
print('Number of layers: ', 4,flush=True)
...
train_model_10layer(train_M,train_E_arr,test_M,test_E_arr,N_units,acts,loss_fn,epochs,batch_size)
print('',flush=True)
