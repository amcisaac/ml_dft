...
def train_model_50layer(X_train,y_train,X_test,y_test,N_units,acts,loss_fn,epochs,batch_size):
    model = Sequential()
    inp_shape = X_train[0].shape[0]
    layer1 = Dense(input_dim = inp_shape ,units=30,activation=acts)
    model.add(layer1)

    for i in range(50):
        layer2 = Dense(units=N_units,activation=acts)
        model.add(layer2)
...
epochs = 25
batch_size = 32
print('Number of layers: ', 50,flush=True)
print('Number of units in layer [1,2,3,4]: ',N_units,flush=True)
print('Activation function in layer [1,2,3,4]: ',acts,flush=True)
print('Loss function: ',loss_fn,flush=True)
print('Number of epochs: ',epochs,flush=True)
...
train_model_50layer(train_M,train_E_arr,test_M,test_E_arr,N_units,acts,loss_fn,epochs,batch_size)
print('',flush=True)
