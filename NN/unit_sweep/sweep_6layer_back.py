...
def train_model_5layer(X_train,y_train,X_test,y_test,N_units,acts,loss_fn,epochs,batch_size):
    ...
    N_units1,N_units2,N_units3,N_units4,N_units5,N_units6 = N_units
...
units = [35,25,15,5]
for N_unit6 in units:
    N_units = [25,15,5,5,15,N_unit6]
    acts = ['relu','relu','relu','relu','relu','relu']
    loss_fn = 'mean_squared_error'
    epochs = 20
    batch_size = 32
    print('Number of layers: ', 6,flush=True)
    print('Number of units in layer [1,2,3,4,5,6]: ',N_units,flush=True)
    print('Activation function in layer [1,2,3,4,5,6]: ',acts,flush=True)
    print('Loss function: ',loss_fn,flush=True)
    print('Number of epochs: ',epochs,flush=True)
    print('Batch size: ',batch_size,flush=True)
    print('',flush=True)

    train_model_6layer(train_M,train_E_arr,test_M,test_E_arr,N_units,acts,loss_fn,epochs,batch_size)
    print('',flush=True)
