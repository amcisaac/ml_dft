...
split0 = 30000 #int(U0.shape[0] * 0.25) # 0.2*133000 ~ 30000
split = len(U0)-split0
energy_type = 'U0'
train_set = U0
test_set = U0
...
for i in range(0,len(y_kr_train)):
    f1.write(train_smiles[i]+','+str(train_Es[i])+','+str(y_kr_train[i])+','+str(y_kr_train[i]-train_Es[i])+','+str(np.abs(y_kr_train[i]-train_Es[i]))+'\n')
f1.close()

f2 = open(output_file_test,'w')
f2.write('SMILES,U0 (DFT),U0 (predicted),Signed error,Absolute Error\n')
for i in range(0,len(y_kr)):
    f2.write(test_smiles[i]+','+str(test_Es[i])+','+str(y_kr[i])+','+str(y_kr[i]-test_Es[i])+','+str(np.abs(y_kr[i]-test_Es[i]))+'\n')
f2.close()
